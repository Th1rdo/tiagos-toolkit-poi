...
# CSS: toda a classe poi-* usada no JS tem de existir na folha de estilo.
...
for grupo in usadas_css:
    classes |= {c for c in grupo.split() if c.startswith("poi-")}
declaradas = set(re.findall(r'\.(poi-[\w-]+)', css)) | set(re.findall(r'#(poi-[\w-]+)', css))
